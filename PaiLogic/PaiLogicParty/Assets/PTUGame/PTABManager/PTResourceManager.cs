...
namespace PTGame.AssetBundles
{
	...
	public class PTResourceManager : PTMonoSingleton<PTResourceManager>
	{
		...
		private const string default_projecttag = "putaogame";


		public PTResourceManager(){
		...
		/// </summary>
		protected IEnumerator Initialize (Action<bool> action,string projectTag, AssetBundleManager manager)
		{
		...
		public void InitializeAsync (Action<bool> action,string projectTag = null)
		{
			...
			if (!_managerDic.ContainsKey(projectTag)) {
				...
				StartCoroutine (Initialize (action, projectTag, _managerDic [projectTag]));
			}
		}
...
//		bool mInitialized = false;
		...
		/// 初始化加载manifest的同步方式
		/// </summary>
		public void Initialize (string projectTag = null)
		{
			projectTag = projectTag == null ? default_projecttag : projectTag;
			string objectName =_defaultGameObjectName+"_"+projectTag;
			if (!_managerDic.ContainsKey(projectTag)) {
		...
		/// </summary>
		/// <param name="assetBundleName">Asset bundle name.</param>
		public AssetBundle LoadAssetBundle (string assetBundleName, string projectTag = null)
		{
			if (string.IsNullOrEmpty (projectTag)) {
			...
			assetBundleName = assetBundleName.ToLower ();
			assetBundleName =RemapAssetBundleName (assetBundleName,projectTag);
			return GetManager(projectTag).LoadAssetBundleSync (assetBundleName,false);
		}
