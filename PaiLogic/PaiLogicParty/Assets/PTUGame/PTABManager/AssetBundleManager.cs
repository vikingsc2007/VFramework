...
//			}
		...
		{
	#if UNITY_EDITOR
			PTDebug.Log ("Simulation Mode: " + (SimulateAssetBundleInEditor ? "Enabled" : "Disabled"));
		...
		{
			PTDebug.Log ("Loading Asset Bundle " + (isLoadingAssetBundleManifest ? "Manifest: " : ": ") + assetBundleName);
	#if UNITY_EDITOR
			...
			if (!isLoadingAssetBundleManifest)
			{
				...
				{
					PTDebug.LogError("Please initialize AssetBundleManifest by calling AssetBundleManager.Initialize()");
					return;
				}
			}
		...
		protected string RemapVariantName(string assetBundleName)
		{
			...
			{
				PTDebug.LogWarning("Ambigious asset bundle variant chosen because there was no matching active variant: " + bundlesWithVariant[bestFitIndex]);
			}
...
//			}
		...
		protected void UnloadAssetBundleInternal(string assetBundleName,bool force)
		{
			...
			if (--bundle.m_ReferencedCount == 0)
			{
				...
				PTDebug.Log(assetBundleName + " has been unloaded successfully");
			}
		}
...
//			assetBundleName= ResetAssetBundleName(assetBundleName);
			PTDebug.Log("Loading "  + assetBundleName + " bundle");
			#if UNITY_EDITOR
			if (SimulateAssetBundleInEditor)
			{
				...
				if (assetPaths.Length == 0)
				{
					PTDebug.LogError("There is no assetbundle with name "+assetBundleName);
					return null;
				}
		...
		{
			PTDebug.Log("Loading " + assetName + " from " + assetBundleName + " bundle");
//			assetBundleName= ResetAssetBundleName(assetBundleName);
			...
			if (SimulateAssetBundleInEditor)
			{
				...
				if (assetPaths.Length == 0)
				{
					PTDebug.LogError("There is no asset with name \"" + assetName + "\" in " + assetBundleName);
					return null;
				}
		...
		{
			PTDebug.Log("Loading " + levelName + " from " + assetBundleName + " bundle");
