...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public void DetachRefFile(string name)
		{
			...
			foreach( KeyValuePair<string, PBXFileReference> current in fileReferences ) 
			{
				if( !string.IsNullOrEmpty( current.Value.name ) && current.Value.name.CompareTo( name ) == 0 ) 
				{
					fileReferences.Remove(current.Key);
					return;
                }
            }
        }
		...
		{	
			...
			{
				string absoluteFilePath = System.IO.Path.Combine( mod.path, file.filePath );
				this.AddFile( absoluteFilePath, file.fileFlag, modGroup );
			}
			...
			if (mod.embed_binaries != null)
			{
				...
				Debug.Log ("***********************************mod.embed_binaries:"+mod.embed_binaries.Count);
