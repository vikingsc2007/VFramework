...
3.加入访问配置文件的接口PTUGame.instance.GetConfigContent（）
4.加入ptdebug
...
2.android 版本 ptgame.jar中增优卸嫌忻挥衟aibot旋转功能，确保在非paibot上也可以运行
...
1.纬度缓存需求更改。如?捍婕吐汲??0条。则新的推送不再推送放入队列后面，同时移除队列第一个元素。
...
缓存是以文本文件记录在本地。
捍嬷械牡谝惶跫锹迹?绻?胤⒋问???，则不在重发，直到接受到一次新的推送请求。则开始重发。
绻?捍婕吐猖?0条。则新耐扑筒辉偻扑停?辉傩慈牖捍妗⑸
失败情况包括：启动时获取user id 不正确；没有网络连接；网络错误。

启动时获取user id 失败。则后续会持?袢。股每隔30秒重新获取一次user id.
如果重新获取的次数超?舷?，则不再获取。
得鞲每突Ф嗣挥辛?油?缁蛘其漱?颍?挥斜匾?偌绦?取?姆研阅堋⑸


--------------2016-04-22-----------------
PTUGame_V2.1.2.unitypackage
1.添加纬度数据上传缓存机制。
a.如果没有连接网络或者发送失败则缓嬖诒镜亍Ｑ?分胤⒄庖惶酰?钡匠晒Α⑸b.根据游戏逻?崴的次序上楚Ｉ弦惶跞绻?О埽?蚝笮?幕嵋来闻哦拥却??钡缴弦条成功、蒫.游戏重启之后。会继续发送上次缓存的数据。新触发的数据，会依次排队到后面。
d.当游戏版本升级的时候，会清楚缓存锹肌⑸e.缓存是以文本文件记录在本地。
2.修改重复获取user id部分。
3.修改socket连接部分。
...
1.如果启动游戏时user id获取失败，则每隔五秒钟再获取一次，直到userid不为－1；
...
2.重构PTUGame结构?奖阏?掀渌?寮??础⑸3.分离出配置文件，岸??虢缑娴淄肌０姹靖?虏毁需瑶婊辉应文箕⑸
----------------2016-2-26---------------
...
1.柚逻LockScreen Canvas Sort Layer 为1000
...
2.ping的发送涓舾奈?。
3.修改家长控制部分逻辑。
4.PTUGame.cs中添加变量isPTUGameReady。
